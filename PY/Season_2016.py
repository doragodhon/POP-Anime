...
def season_2016_analysis():
    ...
    for run in season_dict:
        if season_dict[run] == 'Spring 2016':
            spring_2016.append(run)
        elif season_dict[run] == 'Summer 2016':
            summer_2016.append(run)
        elif season_dict[run] == 'Fall 2016':
            fall_2016.append(run)
        elif season_dict[run] == 'Winter 2016':
            winter_2016.append(run)

    # ---------------------------- Analyzed : Which Anime in Spring is most popular -------------------------
    spring_dict = {}
    spring_list = []
    for run in spring_2016:
    ...
    # ------------------------------- OUTPUT : Spring 2016 Ranking Chart -----------------------------------

    spring_2016_chart = pg.Pie()
    spring_2016_chart.title = 'Top 10 Spring 2016 Ranking Chart'
    for element in spring_tuples[0:11]:
        spring_2016_chart.add(element[0], element[1])
    spring_2016_chart.render_to_file('spring_2016_chart.svg')

    # ------------------------------- OUTPUT : Summer 2016 Ranking Chart -----------------------------------

    summer_2016_chart = pg.Pie()
    summer_2016_chart.title = 'Top 10 Summer 2016 Ranking Chart'
    for element in summer_tuples[0:11]:
        summer_2016_chart.add(element[0], element[1])
    summer_2016_chart.render_to_file('summer_2016_chart.svg')

    # -------------------------------- OUTPUT : Fall 2016 Ranking Chart ------------------------------------

    fall_2016_chart = pg.Pie()
    fall_2016_chart.title = 'Top 10 Fall 2016 Ranking Chart'
    for element in fall_tuples[0:11]:
        fall_2016_chart.add(element[0], element[1])
    fall_2016_chart.render_to_file('fall_2016_chart.svg')

    # ------------------------------- OUTPUT : Winter 2016 Ranking Chart -----------------------------------

    winter_2016_chart = pg.Pie()
    winter_2016_chart.title = 'Top 10 Winter 2016 Ranking Chart'
    for element in winter_tuples[0:11]:
        winter_2016_chart.add(element[0], element[1])
    winter_2016_chart.render_to_file('winter_2016_chart.svg')

season_2016_analysis()
